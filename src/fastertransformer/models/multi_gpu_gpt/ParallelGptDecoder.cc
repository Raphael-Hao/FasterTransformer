...
void ParallelGptDecoder<T>::initialize() {
  ...
  self_attention_layer_ = new TensorParallelDecoderSelfAttentionLayer<T>(
      max_batch_size_, head_num_, size_per_head_, tensor_para_, stream_, cublas_wrapper_,
      allocator_, true, is_free_buffer_after_forward_, sparse_, int8_mode_, custom_all_reduce_comm_,
  ...
  if (activation_type_ == ActivationType::Gelu || activation_type_ == ActivationType::GeGLU) {
    ffn_layer_ = new TensorParallelGeluFfnLayer<T>(
        max_batch_size_, 1, head_num_, size_per_head_,
...
void ParallelGptDecoder<T>::allocateBuffer(size_t batch_size) {
  ...
  normed_self_attn_output_ = reinterpret_cast<T*>(allocator_->reMalloc(
      normed_self_attn_output_, sizeof(T) * batch_size * hidden_units_, false));
  ...
  after_adapter_attn_output_ =
      has_adapters_
          ...
          : self_attn_output_;
  // for moe
  expert_scales_ = reinterpret_cast<T*>(allocator_->reMalloc(
      expert_scales_, sizeof(T) * pad_to_multiple_of_16(moe_k_ * batch_size), false));
  expanded_source_row_to_expanded_dest_row_ = reinterpret_cast<int*>(
      allocator_->reMalloc(expanded_source_row_to_expanded_dest_row_,
                           sizeof(int) * pad_to_multiple_of_16(moe_k_ * batch_size), false));
  expert_for_source_row_ = reinterpret_cast<int*>(allocator_->reMalloc(
      expert_for_source_row_, sizeof(int) * pad_to_multiple_of_16(moe_k_ * batch_size), false));
  ...
  adapter_fc2_result_ =
      has_adapters_
          ? reinterpret_cast<T*>(allocator_->reMalloc(
                adapter_fc2_result_,
                sizeof(T) * pad_to_multiple_of_16(moe_k_ * batch_size * hidden_units_), false))
          : nullptr;
...
template <typename T>
void ParallelGptDecoder<T>::forward(
  ...
  // The resize of the value cache buffer by (local_batch_size, local_head_num, max_seq_len,
  // size_per_head).
  ...
  for (uint l = 0; l < num_layer_; l++) {
    ...
    T* decoder_input =
        (l == 0) ? input_tensors->at("decoder_input").getPtr<T>() : decoder_layer_output_;
    T* decoder_output = (l == num_layer_ - 1) ? output_tensors->at("decoder_output").getPtr<T>()
                                              : decoder_layer_output_;

    if (isFirstLayerParallelId(l) == true && pipeline_para_.rank_ != 0 &&
    ...
    if (layernorm_type_ == LayerNormType::pre_layernorm) {
      invokeGeneralLayerNorm(
          ...
          layer_weight->pre_layernorm_weights.beta, layernorm_eps_, local_batch_size, hidden_units_,
          const_cast<float*>(layer_weight->self_attention_weights.query_weight.scale), int8_mode_,
          stream_);
    }
    ...
    if (input_tensors->count("cache_indirection")) {
      self_attention_input_tensors.insert("cache_indirection",
    ...
    TensorMap self_attention_output_tensors{
        {"hidden_features", Tensor(MEMORY_GPU, activation_out_type,
                                   {local_batch_size, hidden_units_}, self_attn_output_)},
    ...
    if (has_adapters_) {
      invokeGenericActivation<IdentityActivation, T, T>(
          ...
          nullptr, nullptr, nullptr, nullptr, local_batch_size, hidden_units_, 0, nullptr, nullptr,
          stream_);
      ...
      ffn_layer_->forward(&ffn_output_tensors, &ffn_input_tensors,
                          &layer_weight->after_attention_adapter_weights);
    }
    ...
    if (layernorm_type_ == LayerNormType::pre_layernorm) {
      invokeGeneralAddBiasResidualPreLayerNorm(
          ...
          has_adapters_ ? after_adapter_attn_output_ : self_attn_output_, normed_self_attn_output_,
          ...
          layernorm_eps_, local_batch_size, hidden_units_, nullptr, nullptr,
          const_cast<float*>(layer_weight->ffn_weights.intermediate_weight.scale), (float*)nullptr,
          int8_mode_, stream_);
      ...
      invokeAddBiasResidualLayerNorm(
          ...
          layernorm_eps_, local_batch_size, hidden_units_, stream_);
    }
    ...
    if (!use_moe) {
      ffn_output_tensors.insert("ffn_output", Tensor{MEMORY_GPU,
                                                     activation_out_type,
                                                     {local_batch_size, hidden_units_},
                                                     ffn_output_ptr});
    } else {
      ...
      ffn_output_tensors.insert(
          "expert_scales",
          Tensor{MEMORY_GPU, activation_out_type, {local_batch_size, moe_k_}, expert_scales_});
      ffn_output_tensors.insert("expanded_source_row_to_expanded_dest_row",
                                Tensor{MEMORY_GPU,
                                       TYPE_INT32,
                                       {local_batch_size, moe_k_},
    ...
    if (has_adapters_) {
      ...
      if (!use_moe) {
        invokeGenericActivation<IdentityActivation, T, T>(
            ffn_output_ptr, layer_weight->ffn_weights.output_weight.bias, nullptr, nullptr, nullptr,
            nullptr, local_batch_size, hidden_units_, 0, nullptr, nullptr, stream_);
        ...
        invokeGenericActivation<IdentityActivation, T, T>(
            ...
            nullptr, nullptr, moe_k_ * local_batch_size, hidden_units_, 0, nullptr, nullptr,
            stream_);
    ...
    if (!use_moe) {
      if (layernorm_type_ == LayerNormType::pre_layernorm) {
        invokeAddBiasResidual(decoder_output, decoder_output, after_adapter_attn_output_,
                              ...
                              nullptr, nullptr, local_batch_size, hidden_units_, stream_);
      } else if (layernorm_type_ == LayerNormType::post_layernorm) {
        invokeAddBiasResidualLayerNorm(
            ...
            has_adapters_ ? layer_weight->after_ffn_adapter_weights.output_weight.bias
                          : layer_weight->ffn_weights.output_weight.bias,
            layer_weight->self_attn_layernorm_weights.gamma,
            layer_weight->self_attn_layernorm_weights.beta, layernorm_eps_, local_batch_size,
      ...
      if (layernorm_type_ == LayerNormType::pre_layernorm) {
        finalize_moe_routing_kernelLauncher(
            ...
            local_batch_size, hidden_units_, moe_k_, stream_);
      } else if (layernorm_type_ == LayerNormType::post_layernorm) {
        finalize_moe_routing_kernelLauncher(
            fc2_result_, decoder_output, after_adapter_attn_output_,
            has_adapters_ ? layer_weight->after_ffn_adapter_weights.output_weight.bias
        ...
        invokeGeneralLayerNorm(decoder_output, decoder_output,
                               ...
                               local_batch_size, hidden_units_, (float*)nullptr, 0, stream_);
      }
    }
    sync_check_cuda_error();
    POP_RANGE;
    ...
    if (isLastLayerParallelId(l) == true &&
        pipeline_para_.rank_ != pipeline_para_.world_size_ - 1 && pipeline_para_.world_size_ > 1) {
      ftNcclSend(decoder_output + local_batch_size * hidden_units_ / tensor_para_.world_size_ *
                                      tensor_para_.rank_,
