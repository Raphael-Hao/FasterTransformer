...
int read_start_ids(size_t batch_size,
  ...
  for (uint i = 1; i < (uint)tmp_start_lengths.size(); i++) {
    max_input_len =
        max_input_len > tmp_start_lengths.data()[i] ? max_input_len : tmp_start_lengths.data()[i];
  }
...
model_config_t read_model_config(const INIReader& reader) {
  ...
  config.use_ffn = reader.GetBoolean("ft_instance_hyperparameter", "use_ffn", true);
  ...
  const std::string model_variant =
      std::string(reader.Get(config.model_name, "model_variant", "gpt"));
  ...
  config.prompt_learning_type = static_cast<PromptLearningType>(
      reader.GetInteger(config.model_name, "prompt_learning_type", 0));
...
request_config_t read_request_config(const INIReader& reader) {
  ...
  config.repetition_penalty =
      reader.GetFloat("ft_instance_hyperparameter", "repetition_penalty", 1.0f);
  ...
  FT_CHECK_WITH_INFO(
      ...
      fmtstr("Found ambiguous parameters repetition_penalty (%f) and presence_penalty (%f) "
             "which are mutually exclusive. Please remove one of repetition_penalty or "
             "presence_penalty "
             "or set to a default value.",
             config.repetition_penalty, config.presence_penalty));
  ...
  config.shared_contexts_ratio =
      reader.GetFloat("ft_instance_hyperparameter", "shared_contexts_ratio", 1.0f);
...
std::map<std::string, std::pair<int, int>> init_prompt_tuning_map(const INIReader& reader,
  ...
  // NOTE：specify task names, take name id, prompt length in order to load those prompt learning
  // tables. for example: std::map<std::string, std::pair<int, int>>
  // p_prompt_tuning_table_pair_{{"sentiment", {0, 10}},
  //                                                                        {"intent_and_slot", {1,
  //                                                                        10}},
  //                                                                        {"squad", {2, 16}}};
...
std::pair<int, int> init_multiprocessing(const model_config_t& model_config) {
  ...
  if (layers_per_group * model_config.pipeline_para_size != (int)model_config.decoder_layers) {
    printf(
        "[ERROR] layers_per_group (%d) * pipeline_para_size (%d) should equal to decoder_layers "
        "(%ld) \n",
        layers_per_group, model_config.pipeline_para_size, model_config.decoder_layers);
    exit(-1);
  }
...
void init_nccl(const model_config_t& model_config,
               NcclParam& tensor_para,
               NcclParam& pipeline_para) {
  // assume gpu_num = k * n,
  // tensor parallelism group size is n
  // pipeline parallelism group size is k
  ftNcclInitialize(tensor_para, pipeline_para, model_config.tensor_para_size,
...
cublasMMWrapper init_cublas_ctx(DataType data_type,
  ...
  cublasMMWrapper cublas_wrapper =
      cublasMMWrapper(cublas_handle, cublaslt_handle, cusparselt_handle, stream, &cublas_algo_map,
                      &cublas_wrapper_mutex, &allocator);
#else
...
void populate_request(std::unordered_map<std::string, Tensor>& input_tensors,
  ...
  std::vector<int> v_start_ids;
  request_batch_size = read_start_ids(request_batch_size, &v_start_lengths, &v_start_ids,
                                      max_input_len, model_config.end_id, 1, csv_path);
  ...
  input_tensors.insert(
      {"input_ids", {MEMORY_GPU, TYPE_INT32, {request_batch_size, max_input_len}, d_input_ids}});
  input_tensors.insert(
      {"input_lengths", {MEMORY_GPU, TYPE_INT32, {request_batch_size}, d_input_lengths}});
  input_tensors.insert(
      {"input_lengths_h", {MEMORY_CPU, TYPE_INT32, {request_batch_size}, v_start_lengths.data()}});
  input_tensors.insert(
      {"output_seq_len", {MEMORY_CPU, TYPE_UINT32, {request_batch_size}, output_seq_len.data()}});
  ...
  if (request_config.repetition_penalty != 1.0f) {
    input_tensors.insert(
        {"repetition_penalty", {MEMORY_CPU, TYPE_FP32, {1}, &request_config.repetition_penalty}});
  }
  if (request_config.presence_penalty != 0.0f) {
    input_tensors.insert(
        {"presence_penalty", {MEMORY_CPU, TYPE_FP32, {1}, &request_config.presence_penalty}});
  }
  ...
  if (request_config.top_k == 0 && request_config.top_p == 0.0f) {
    ...
    input_tensors.insert(
        {"beam_search_diversity_rate",
         {MEMORY_CPU, TYPE_FP32, {1}, &request_config.beam_search_diversity_rate}});
  ...
  if (request_config.memory_len > 0) {
    input_tensors.insert(
        {"memory_len", {MEMORY_CPU, TYPE_UINT32, {1}, &request_config.memory_len}});
  }
  ...
  deviceMalloc(&d_output_ids, request_batch_size * request_config.beam_width * total_output_len,
               false);
  deviceMalloc(&d_sequence_lengths, request_batch_size * request_config.beam_width, false);

  output_tensors.insert(
      {"output_ids",
       {MEMORY_GPU, TYPE_INT32, {request_batch_size, beam_width, total_output_len}, d_output_ids}});
  output_tensors.insert(
      {"sequence_length",
       {MEMORY_GPU, TYPE_INT32, {request_batch_size, beam_width}, d_sequence_lengths}});
  ...
  if (request_config.is_return_log_probs) {
    ...
    output_tensors.insert({"output_log_probs",
                           {MEMORY_GPU,
                            TYPE_FP32,
                            {request_batch_size, beam_width, request_output_len},
                            output_log_probs}});
    output_tensors.insert(
        {"cum_log_probs",
         {MEMORY_GPU, TYPE_FP32, {request_batch_size, beam_width}, d_cum_log_probs}});
  }

  if (request_config.is_return_context_embeddings) {
    deviceMalloc(&output_context_embeddings,
...
void write_output_tensors(std::unordered_map<std::string, Tensor>& output_tensors) {
  ...
  if (output_tensors.count("cum_log_probs")) {
    ...
    if (!logprob_file.is_open()) {
      ...
      cudaD2Hcpy(h_buf.data(), output_tensors.at("cum_log_probs").getPtr<float>(),
                 cum_log_probs_size);
