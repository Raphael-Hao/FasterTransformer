...
void multi_gpu_gpt_example(const INIReader reader, std::string in_csv) {
  ...
  std::map<std::string, std::pair<int, int>> p_prompt_tuning_table_pair(
      init_prompt_tuning_map(reader, model_config));
  ...
  cublasMMWrapper cublas_wrapper =
      init_cublas_ctx(data_type, stream, allocator, cublas_handle, cublas_wrapper_mutex,
                      cublaslt_handle, cublas_algo_map
#ifdef SPARSITY_ENABLED
                      ,
                      cusparselt_handle
#endif
  ...
  ParallelGptWeight<T> gpt_weights(
      model_config.hidden_units, model_config.inter_size, model_config.vocab_size,
      model_config.decoder_layers, model_config.max_seq_len, tensor_para.world_size_,
      tensor_para.rank_, pipeline_para.world_size_, pipeline_para.rank_, model_config.int8_mode,
      model_config.prompt_learning_type, p_prompt_tuning_table_pair, model_config.gpt_variants);
  ...
  AttentionType attention_type =
      getAttentionType<T>(model_config.size_per_head, getSMVersion(),
  ...
  ParallelGpt<T> gpt =
      ParallelGpt<T>(0,  // max_batch_size, FT will adjust the buffer automatically.
                     ...
                     model_config.head_num, model_config.size_per_head, model_config.inter_size,
                     model_config.decoder_layers,
                     0,   // expert_num
                     0,   // moe_k
                     {},  // moe_layer_index
                     model_config.vocab_size, model_config.start_id, model_config.end_id,
                     // p/prompt tuning virtual token start id
                     model_config.prompt_learning_start_id, model_config.prompt_learning_type,
  ...
  populate_request(input_tensors, output_tensors, d_input_ids, d_input_lengths, v_start_lengths,
                   output_seq_len, p_prompt_tuning_task_ids, d_output_ids, d_sequence_lengths,
                   d_output_log_probs, d_cum_log_probs, d_output_context_embeddings, random_seed,
                   in_csv, model_config, request_config);
  print_mem_usage();
  ...
  printf(
      "[INFO] request_batch_size %ld beam_width %ld head_num %ld size_per_head %ld "
      "total_output_len %ld"
      " decoder_layers %ld vocab_size %ld FT-CPP-decoding-beamsearch-time %.2f ms\n",
      request_config.request_batch_size, request_config.beam_width, model_config.head_num,
      model_config.size_per_head, total_output_len, model_config.decoder_layers,
